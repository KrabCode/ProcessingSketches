...
public class Boids extends HotswapGuiSketch {
    ...
    private ArrayList<IParticle> particles = new ArrayList<IParticle>();
    private ArrayList<IParticle> particlesToRemove = new ArrayList<IParticle>();
    private float bloodAcc;
    private int bloodRes;
...
//        fullScreen(P2D, 2);
    ...
    public void draw() {
        ...
        mouse.x = mouseX - cameraOffset.x;
        mouse.y = mouseY - cameraOffset.y;
        pg.beginDraw();
        sliders();
        seaPass();
        updateCamera();
    ...
    private void sliders() {
        ...
        drag = slider("drag", .5f, 1);
        maxRot = slider("max rotation", 6);
        scl = slider("scl", 3.2f);
        minSpd = slider("minimum speed", 5);
        bloodRes = floor(slider("blood res", 18));
        bloodAcc = slider("blood acc", 10);
        if (frameCount == 1 || toggle("rules")) {
            ...
            centerRadius = slider("center r", 240);
            centerMag = slider("center mag", .01f);
            alignRadius = slider("align r", 220);
    ...
    private void updateCamera() {
        ...
        this.pg.translate(cameraOffset.x, cameraOffset.y);
    }
...
//                    println("boid died because he was far behind the player: ", boid.pos, " - ", player.pos);
            ...
            boid.display(t);
            if (mousePressedOutsideGui && isBoidInRectangle(boid, mouse, 20)) {
                boid.dead = true;
    ...
    private ArrayList<IParticle> bloodParticleCloud(PVector pos) {
        ArrayList<IParticle> cloud = new ArrayList<IParticle>();
        int finalRes = floor(bloodRes+random(-3,3));
        for (int i = 0; i < finalRes; i++) {
            float inorm = norm(i, 0, finalRes - 1);
            float angle = inorm * TWO_PI;
            ParticleBlood drop = new ParticleBlood(this, pg);
            drop.pos.x = pos.x;
            drop.pos.y = pos.y;
            drop.acc.add(PVector.fromAngle(angle).mult(bloodAcc));
            cloud.add(drop);
        }
        return cloud;
    }

    private ArrayList<IParticle> featherParticleCloud(PVector pos) {
        return new ArrayList<IParticle>();
    }
    ...
    private void updatePlayerTarget() {
        ...
        */
        ...
        if (input.rightStickMagnitude > .18) {
            targetActive = true;
            playerTarget.x = player.pos.x + input.rightStickX;
            playerTarget.y = player.pos.y - input.rightStickY;
        } else {
            targetActive = false;
        }
    }
    ...
    private void updateBoidCount() {
        int intendedBoidCount = floor(slider("count", 1, 101));
        while (boids.size() < intendedBoidCount) {
            boids.add(new Boid(this, pg, randomPositionOffscreenInFrontOfPlayer()));
        }
        while (boids.size() > intendedBoidCount) {
    ...
    private boolean isBoidInRectangle(Boid a, PVector center, float range) {
        return isPointInRect(center.x, center.y,
                a.pos.x - range,
                a.pos.y - range,
                range * 2,
                range * 2);
    ...
    private void displayDebugCircle(Boid boid, float radius) {
        ...
        pg.ellipse(boid.pos.x, boid.pos.y, radius * 2, radius * 2);
    }
    ...
    private void displayDebugArc(Boid boid, float radius, float angularSize) {
        ...
        pg.pushMatrix();
        pg.translate(boid.pos.x, boid.pos.y);
        pg.rotate(boid.spd.heading());
        pg.noFill();
        pg.ellipseMode(CENTER);
        pg.arc(0, 0, radius * 2, radius * 2, -angularSize * .5f, angularSize * .5f, PIE);
