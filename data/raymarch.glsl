...
uniform int maxSteps = 150;
uniform float maxDist = 150.;
uniform float surfaceDist = 0.001;
...
dist getDistance(vec3 p){
    ...
    for (int i = 0; i < 10; i++){
        vec3 ip = p.xyz;
        if (mod(i, 2) == 0){
            ip.xy *= rotate2d(time+i*.5);
        } else {
            ip.yz *= rotate2d(time-i*.5);
        }
        float outer = sdCappedCylinder(ip, baseRadius+i*step, height);
        float inner = sdCappedCylinder(ip, baseRadius+i*step-width, height*2);
        d = min(d, opSubtraction(inner, outer));
    }
    float s = sdSphere(p, 0.5);
    return dist(min(s, d), 0);
}
