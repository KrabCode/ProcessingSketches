...
void main(){
    ...
    float sunRadius = .12;
    float ad = angularDiameter(d, sunRadius*.9);
    cv *= clamp(d, sunRadius*.7, sunRadius)*120;

    float noise = fbm(
                cv.x - t,
                cv.y,
                noiseTimeRadius*cos(t),
                noiseTimeRadius*sin(t)
    );

    float timelessNoise = fbm(
        cv.x,
        cv.y,
        noiseTimeRadius*cos(t),
        noiseTimeRadius*sin(t)
    ...
    col += ad;
    col -= cubicPulse(0., sunRadius, d) * noise;
