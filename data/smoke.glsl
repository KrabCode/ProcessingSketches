...
void main(){
    ...
    vec2 id = floor(cv)+.5;
    id.y -= t;
    float pct = cubicPulse(1., .8, abs(sin(id.y)))  * cubicPulse(1., 1.0, abs(sin(id.x)));
    float center = smoothstep(0., 80., length(cv));
    col += center;
    col.r += pct*.3;
    col.g += pct*.4;
    col.b += pct*2.;
    ...
    uv = vec2(fbm(uv.x, uv.y), fbm(pow(uv.y, 5.), pow(uv.x, .5)));
//    col *= fbm(uv.x, uv.y, t);
