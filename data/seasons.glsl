...
void main(){
    ...
    uv.y = 1.-uv.y;
    vec3 color_0 = texture(img_0, uv).rgb;
    vec3 color_1 = texture(img_1, uv).rgb;
    vec3 color_2 = texture(img_2, uv).rgb;
    vec3 color_3 = texture(img_3, uv).rgb;
    vec3[] colors = vec3[4](color_0, color_1, color_2, color_3);

    uv *= 0.1;
    float n = clamp(.5+.5*fbm(uv.x, uv.y, tr*cos(t), tr*sin(t)), 0, 1);
    n = n*4;

    vec3 color;

    if(n > 3){
        color = mix(colors[3], colors[0], fract(n));
    }else if(n > 2){
        color = mix(colors[2], colors[3], fract(n));
    }else if(n > 1){
        color = mix(colors[1], colors[2], fract(n));
    }else{
        color = mix(colors[0], colors[1], fract(n));
    }
