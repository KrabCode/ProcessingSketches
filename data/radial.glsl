...
void main(){
    ...
    float d = length(cc);
    float a = atan(cc.y, cc.x)-d-t;
    float r = 10.;
    float fbmScl = 1.;
    float n = fbm(vec4(uv.x*fbmScl, uv.y*fbmScl, cos(d), t));
    float dir = a;
    ...
    float scl = 1.;
    float pct = cubicPulse(0., 0.2, d);
    c += rgb(vec3(.1*pct, 1., pct));
    c *= 1.+.1*(sin(uv.x*scl)*sin(uv.y*scl));
    c -= .05;
