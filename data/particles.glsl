...
float wind(vec3 p) {
    ...
    return length(p)*(0.9+d);
}

void main(){
    vec2 cv = gl_FragCoord.xy / resolution.xy - .5;
    cv *= 0.3;
    vec3 dir = vec3(cv, 1.0);
    ...
    for (float r= 0.; r<steps; r++) {
        ...
        v+=min(200., wind(p)) * max(0., 1.-r*fade);
    }
    v/=steps;
    v*=brightness;
    vec3 col= vec3(pow(v, 4.));
    col *= 1.-length(pow(abs(cv), vec2(5.)))*30;// vignette (kind of)
